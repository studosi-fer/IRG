/* treci zadatak - julijev fraktal

g++ zad3b.cpp -lglut -lGLU -o zad3b && ./zad3b
...
float umin, umax, vmin, vmax, c_re, c_im;
int maxit;
int eps;
...
int main(int argc, char *argv[]) {
	...
	cout << "Unesite realni i imaginarni dio konstante c: ";
	cin >> c_re >> c_im;
	...
	glutCreateWindow("Vjezba 3 - Juliev");
...
void display() {
	...
	glBegin(GL_POINTS);
	float r, z_re, z_im, z_re2, z_im2;
	int k;
	for (int y = 0; y < height; y++) {
		for (int x = 0; x < width; x++) {
			z_re = (umax-umin)*x / (float) width + umin;
			z_im = (vmax-vmin)*y / (float) height + vmin;
			k = -1;
